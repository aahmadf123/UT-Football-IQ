...
| **Storage** | Raw exported images/labels stay **local / gitignored**; if ever needed at scale they go to the object store `artifacts/`. **Never commit images or `.pt`/`.onnx`/`.safetensors` weights** (`.gitignore` enforces). | Raw JSON/CSV/Parquet stays **local / external**, **gitignored**; do not commit the dump. |
